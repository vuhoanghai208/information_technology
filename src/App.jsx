...
// ============================================================================
// ⚠️ QUAN TRỌNG: CẤU HÌNH RAG (AI ĐỌC TÀI LIỆU)
// ============================================================================
// Bước 1: Nếu bạn ĐÃ CHẠY Python và có file 'knowledge_vectors.js', hãy BỎ COMMENT dòng dưới:
import { PRE_CALCULATED_KNOWLEDGE as VECTOR_DATA } from './knowledge_vectors_min';

// Bước 2: Khởi tạo biến dữ liệu (Tự động nhận diện)
let PRE_CALCULATED_KNOWLEDGE = [];

// Kiểm tra xem đã import được chưa
try {
  // Hack nhỏ để kiểm tra biến import mà không gây lỗi build
  if (typeof VECTOR_DATA !== 'undefined') {
    PRE_CALCULATED_KNOWLEDGE = VECTOR_DATA;
    console.log("✅ Đã nạp thành công dữ liệu Vector RAG!");
  } else {
    console.log("ℹ️ Đang chạy chế độ thường (Chưa có dữ liệu Vector).");
  }
} catch (e) {
  // Lờ đi nếu chưa import
}
// ============================================================================
...
// ============================================================================
// TÍNH NĂNG MỚI 1: GAME GHÉP CỘT (MATCHING)
// ============================================================================
const MatchingGame = ({ question, onComplete }) => {
  ...
  useEffect(() => {
    // Xáo trộn vị trí khi load câu hỏi
            ...
            {leftItems.map(item => {
              ...
              return (
                <button key={item.id} onClick={() => handleLeftClick(item.id)} disabled={isMatched}
                  ...
                  {item.text}
                  {isSelected && <div className="absolute -right-2 top-1/2 w-2 h-2 bg-cyan-400 rounded-full animate-ping"></div>}
                </button>
              )
            })}
         </div>
...
// ============================================================================
// TÍNH NĂNG MỚI 2: GAME DỰ ĐOÁN (PREDICT)
// ============================================================================
    ...
    <div className="w-full max-w-2xl mx-auto animate-in fade-in">
      ...
      <form onSubmit={handleNext} className="space-y-6">
        <div className={cn("transition-all duration-500", step > 1 ? "opacity-50 pointer-events-none grayscale" : "")}>
          ...
          <input type="text" value={userPredict} onChange={e => setUserPredict(e.target.value)} disabled={step > 1}
            className="w-full bg-[#1e293b] border border-slate-600 rounded-xl p-4 text-white focus:border-cyan-500 outline-none font-mono" placeholder="Ví dụ: 10, Error, True..." />
        </div>
        ...
        {step >= 2 && (
           <div className={cn("animate-in slide-in-from-bottom-4 fade-in duration-500", step > 2 ? "opacity-50 pointer-events-none grayscale" : "")}>
             ...
             <textarea value={userReason} onChange={e => setUserReason(e.target.value)} disabled={step > 2}
               className="w-full bg-[#1e293b] border border-slate-600 rounded-xl p-4 text-white focus:border-yellow-500 outline-none min-h-[80px]" placeholder="Giải thích ngắn gọn..." />
           </div>
        )}
        ...
        {step < 3 && (
          <button type="submit" className="w-full py-4 bg-gradient-to-r from-cyan-600 to-blue-600 rounded-xl font-bold text-white shadow-lg hover:scale-[1.02] transition-all">
            {step === 1 ? "Xác nhận dự đoán" : "Xem kết quả"}
          </button>
        )}
      </form>
    </div>
...
// ============================================================================
// COMPONENT CHAT AI (GIAO DIỆN ĐẸP + FORMAT VĂN BẢN) - GIỮ NGUYÊN
// ============================================================================
const AIChat = () => {
  ...
  const handleSetKey = (e) => {
    ...
    if (apiKey.trim().length > 10) {
      setIsKeySet(true);
      setMessages(prev => [...prev, { role: 'assistant', content: '✅ Đã kết nối OpenAI! Sẵn sàng hỗ trợ.' }]);
    }
  };
  ...
  const renderContent = (text) => {
    return text.split('\n').map((line, i) => {
      if (line.trim().startsWith('-') || line.trim().startsWith('•')) {
        return ( <div key={i} className="flex gap-2 ml-1 mb-1"><span className="text-cyan-400 font-bold">•</span><span>{line.substring(1).trim()}</span></div> );
      }
  ...
  const handleSend = async (e) => {
    ...
    try {
      let systemPrompt = "Bạn là trợ lý IT Tutor. Trình bày câu trả lời rõ ràng, chia đoạn, gạch đầu dòng.";
      if (isKeySet && PRE_CALCULATED_KNOWLEDGE.length > 0) { /* RAG logic */ }

      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` },
        body: JSON.stringify({ model: "gpt-3.5-turbo", messages: [{ role: "system", content: systemPrompt }, ...messages.slice(-4), userMsg] })
      });
        ...
        <div className="w-[90vw] md:w-[400px] h-[600px] bg-[#0f172a]/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 fade-in duration-300 ring-1 ring-white/10">
          <div className="p-4 border-b border-white/5 bg-gradient-to-r from-blue-900/40 to-cyan-900/40 flex justify-between items-center backdrop-blur-md">
            <div className="flex items-center gap-3">
              <div className="relative"><div className="w-2 h-2 absolute top-0 right-0 bg-green-500 rounded-full animate-pulse"></div><Bot size={24} className="text-cyan-400" /></div>
              <div><span className="font-bold text-slate-100 block text-sm">AI Mentor</span><span className="text-[10px] text-cyan-400 uppercase tracking-wider font-bold">Online</span></div>
            </div>
            <button onClick={() => setIsOpen(false)} className="p-2 hover:bg-white/10 rounded-full transition-colors text-slate-400 hover:text-white"><X size={18} /></button>
          </div>
          <div className="flex-1 overflow-y-auto p-5 space-y-6 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
            {messages.map((msg, idx) => (
              <div key={idx} className={cn("flex gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300", msg.role === 'user' ? "flex-row-reverse" : "flex-row")}>
                <div className={cn("w-8 h-8 rounded-full flex items-center justify-center shrink-0 shadow-lg mt-1", msg.role === 'user' ? "bg-gradient-to-br from-blue-500 to-indigo-600" : "bg-gradient-to-br from-cyan-500 to-teal-600")}>
                  {msg.role === 'user' ? <User size={14} className="text-white"/> : <Zap size={14} className="text-white fill-white"/>}
                </div>
                <div className={cn("px-4 py-3 rounded-2xl text-[15px] max-w-[85%] shadow-md", msg.role === 'user' ? "bg-blue-600 text-white rounded-tr-sm" : "bg-[#1e293b] text-slate-200 border border-slate-700/50 rounded-tl-sm")}>
          ...
          <div className="p-4 bg-[#020617]/40 border-t border-white/5 backdrop-blur-md">
            {!isKeySet ? (
              <form onSubmit={handleSetKey} className="flex gap-2">
                <div className="relative flex-1"><Key size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500"/><input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="Nhập API Key OpenAI..." className="w-full bg-slate-900/80 border border-slate-700 rounded-xl py-3 pl-9 pr-4 text-sm text-white focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none transition-all placeholder:text-slate-600" /></div>
                <button type="submit" className="bg-cyan-600 hover:bg-cyan-500 text-white px-4 rounded-xl font-bold text-sm transition-all shadow-lg shadow-cyan-500/20">Lưu</button>
              </form>
            ) : (
              <form onSubmit={handleSend} className="flex gap-2">
                <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Hỏi gì đó..." className="flex-1 bg-slate-900/80 border border-slate-700 rounded-xl px-4 py-3 text-sm text-white focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none transition-all placeholder:text-slate-600" />
                <button type="submit" disabled={isLoading} className="bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 disabled:grayscale text-white p-3 rounded-xl transition-all shadow-lg shadow-cyan-500/20 transform active:scale-95"><Send size={18} /></button>
              </form>
            )}
          </div>
        </div>
      )}
      <button onClick={() => setIsOpen(!isOpen)} className="w-14 h-14 rounded-full bg-gradient-to-tr from-cyan-500 to-blue-600 flex items-center justify-center shadow-[0_0_20px_rgba(6,182,212,0.4)] hover:scale-110 hover:shadow-[0_0_30px_rgba(6,182,212,0.6)] transition-all duration-300 active:scale-95 group z-50 border border-white/10">
...
// ============================================================================
// LOGIC GAME CHÍNH (ĐÃ CẬP NHẬT ĐỂ HỖ TRỢ 2 GAME MỚI)
// ============================================================================
const TOPICS = [
...
const ITQuizGame = () => {
  ...
  const startExam = (topic) => {
    ...
    const preparedQuestions = questions.map(q => {
      // Nếu là Matching hoặc Predict thì giữ nguyên
  ...
  // Handler khi hoàn thành minigame
  const handleSpecialComplete = (isCorrect) => {
      if (isCorrect) setScore(s => s + 20); 
      if (activeQuestions[currentQIndex].type === 'match') {
    ...
    return (
      <div className="flex flex-col items-center justify-center my-6 animate-in zoom-in duration-500">
        <div className="relative group">
            <div className="w-28 h-28 md:w-36 md:h-36 rounded-full shadow-[0_0_30px_rgba(0,0,0,0.5)] border-4 border-white/20 transition-transform group-hover:scale-110 z-10 relative" style={{ backgroundColor: colorCode }}></div>
            <div className="absolute inset-0 rounded-full blur-xl opacity-60 -z-0" style={{ backgroundColor: colorCode }}></div>
        </div>
        <div className="mt-4 px-4 py-1 bg-black/40 backdrop-blur-md rounded-lg border border-white/10 font-mono text-lg text-cyan-300">{colorCode}</div>
      </div>
    );
  };
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#050511] text-white font-sans overflow-x-hidden flex flex-col items-center selection:bg-cyan-500/30">
      ...
      <main className="relative z-10 w-full flex-1 flex flex-col justify-center py-8">
        ...
        {/* MENU MODE (GIỮ NGUYÊN MARQUEE CHẠY CHỮ) */}
        {mode === 'menu' && (
        ...
        {/* PLAYING MODE (THÊM 2 GAME MỚI VÀO ĐÂY) */}
        {mode === 'playing' && currentQ && (
          <div className="w-full max-w-4xl mx-auto animate-in zoom-in-95 px-4 duration-500">
            <div className="bg-[#0f172a]/40 border border-white/10 p-8 md:p-12 rounded-[2.5rem] backdrop-blur-2xl shadow-2xl relative overflow-hidden">
              ...
              {currentQ.colorCode && (
                  <div className="flex justify-center my-6"><div className="w-28 h-28 rounded-full border-4 border-white/20 shadow-[0_0_30px_rgba(0,0,0,0.5)]" style={{ backgroundColor: currentQ.colorCode }}></div></div>
              )}

              <h3 className="text-2xl md:text-3xl font-black mb-10 text-center leading-snug drop-shadow-lg">{currentQ.q}</h3>

              {/* === PHẦN LOGIC RENDER CÁC LOẠI CÂU HỎI === */}
              
              {currentQ.type === 'match' ? (
                  <MatchingGame question={currentQ} onComplete={handleSpecialComplete} />
              ) : 
              currentQ.type === 'predict' ? (
                  <PredictGame question={currentQ} onComplete={handleNextQuestion} />
              ) :
              currentQ.type === 'text' ? (
                ...
                <div className={cn("grid gap-4 mb-8 animate-in fade-in slide-in-from-bottom-4", currentQ.type === 'tf' ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1")}>
                    {currentQ.options.map((opt, i) => {
                        ...
                        if (isAnswered) {
                            if (isCorrect) btnClass = "bg-green-500/20 border-green-500 text-green-300 ring-1 ring-green-500/50";
                            else if (isSelected) btnClass = "bg-red-500/20 border-red-500 text-red-300 ring-1 ring-red-500/50";
                            else btnClass = "bg-white/5 opacity-30 blur-[1px]";
                        }
                        return (
                            <button key={i} disabled={isAnswered} onClick={() => handleOptionSelect(opt)} className={cn("w-full p-6 rounded-2xl border text-left text-lg font-bold transition-all flex justify-between items-center group relative overflow-hidden", btnClass)}>
                                <span className="relative z-10">{opt}</span>
                                {isAnswered && isCorrect && <Check size={28} className="text-green-400 relative z-10 animate-bounce" />}
                                {isAnswered && isSelected && !isCorrect && <X size={28} className="text-red-400 relative z-10" />}
                            </button>
                        );
                    })}
                </div>
              ...
              {/* GIẢI THÍCH (Chỉ hiện cho text/mc/tf - Game Matching/Predict tự xử lý riêng) */}
